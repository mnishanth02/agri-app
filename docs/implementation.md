...
3. Add `apps/api/src/services/eosda-stats.ts` exporting `runMtStats({ geometry, indexes, dateRange, log, signal })` (mirrors the `eosda-search.ts` pattern from Phase 4). The service owns:
   ...
   - **Per-scene errors are NOT fatal.** EOSDA `mt_stats` returns `{ result: [...], errors: [...] }` simultaneously when some scenes succeed and others fail (e.g., `"AOI contains clouds only"` for a single date). The poll loop logs `errors[]` as a `warn` and returns the successful `result[]` rows. Only when `errors[]` exists with no `result[]` (or `result[]` is empty) do we throw — and even then the route falls back to stale cache before returning 502. This is critical: treating per-scene cloud errors as fatal causes the entire `mt_stats` call to short-circuit, downgrade to an empty cache read, and surface as `502 EOSDA mt_stats unavailable` to the UI.
